...
- 修复范围：
  ...
  - [x] 2026-07-13 固定场景 Harness 第二阶段覆盖注册表中的全部当前非 PoW 题型：曲线描绘、滑动曲线 V3、轮廓滑块、旋转、滑动还原、角度、刮擦、文字点选和图标点选；注册表覆盖测试保证新增题型未加入 Harness 时门禁失败。隐藏答案仅在 `internal/captcha` 的 `_test.go` 进程内读取，Node 只解析题型名和四个布尔状态；正式 API、路由和发布二进制无解题入口。全题型 runner 与 `-race` 已通过。
  ...
  - [x] 2026-07-15 修正浏览器 Harness 的 Zustand 持久化键后，9 种非 PoW 题型在桌面/移动、浅色/深色、中英文共 72 个真实 Playwright 场景通过；每项覆盖错误物理交互、约一秒换题、旧 token 重放 410、正确物理交互、正确 token 重放 410 和成功冻结。登录与 WAF 质询的桌面/移动四条接入闭环同时通过，显式刷新、关闭和取消由组件、运行期及进程生命周期测试覆盖。
...
- 结果：2026-07-15 私有 Harness 的 24 项 Node 门禁与登录/WAF fixture 的 6 项独立生命周期测试均通过；正在将后者纳入 GitHub、Forgejo 的固定门禁，避免只在本地单独执行。
...
### [~] BUG-074 P1 滑动曲线公开参数可直接计算答案且轨迹统计误伤真实浏览器

- 证据：V3 题面同时公开 `initial_offset` 与 `max_offset`，可用 `5000 - initial_offset*5000/max_offset` 直接得到服务端目标；新增的固定步长/时间方差硬拒绝又会把 Chromium 真实 16–17ms 采样轨迹判为脚本。
- 影响：公开接口可绕过视觉对齐；加固后反而会拒绝正常用户，与低误报优先原则冲突。
- 修复：公开协议移除数值偏移，只保留视觉对齐任务；初始位移、目标和几何一致性保留在 AEAD token。服务端校验中点起始、终值、方向、回折、路径效率和配置时长，不再把统计方差作为通过硬门槛；前端使用同步 ref 消除松手竞态并补键盘、触屏取消和本地化名称。视觉题面本身承载人类要完成的匹配信息，不将其表述为密码学秘密。
- 验收：公开 presentation 不含可直接代入公式的数值偏移，位图边界也不能直接给出位移；畸形密封几何、两点跳跃、未覆盖配置时长、折返、错误目标和低于配置时长均被拒；桌面/移动物理轨迹在量化误差 24/32 下通过，初始/对齐截图位于 `output/playwright/captcha-review/`。

### [~] BUG-075 P2 CAPTCHA 浏览器矩阵的主题与语言维度实际未生效

- 证据：旧 `prepareLab` 写入 `cheesewaf-theme`、`cheesewaf-language`，应用实际从 Zustand 的 `cheesewaf-ui` 恢复设置；标记为深色英文的截图仍是浅色中文。
- 影响：72/88 场景日志虽显示不同主题和语言，实际重复验证默认浅色中文，形成虚假覆盖。
- 修复：Harness 按 Zustand persist 结构写入 `cheesewaf-ui`，保留 i18next 初始化键；新增 Node 回归测试禁止重新使用废弃键。
- 验收：深色英文截图实际显示英文文案和深色壳层；24 项 Node 门禁与 72 个浏览器场景通过，无横向溢出、控制台错误或请求失败。
