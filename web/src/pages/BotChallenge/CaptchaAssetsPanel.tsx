...
export default function CaptchaAssetsPanel() {
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: () => { toast.success(t('botChallenge.captchaAssets.uploadSuccess')); void queryClient.invalidateQueries({ queryKey: ['captcha-assets'] }); },
    onError: (error) => toast.error(errorMessage(error, t('botChallenge.captchaAssets.uploadFailed'))),
  });
  const deleteMutation = useMutation({
    mutationFn: deleteCAPTCHAAsset,
    onSuccess: () => {
      toast.success(t('botChallenge.captchaAssets.deleteSuccess'));
      void queryClient.invalidateQueries({ queryKey: ['captcha-assets'] });
      setDeleteTarget(null);
    },
    onError: (error) => toast.error(errorMessage(error, t('botChallenge.captchaAssets.deleteFailed'))),
  });
  const previewMutation = useMutation({
    ...
    onError: (error) => toast.error(errorMessage(error, t('botChallenge.captchaAssets.previewFailed'))),
  });
  const saveMutation = useMutation({
    mutationFn: updateCAPTCHAAssetConfig,
    onSuccess: (saved) => { setConfig(toUpdate(saved)); setCredentialConfigured(saved.s3.credential_configured); setMetadataKeyConfigured(saved.s3.metadata_key_configured); toast.success(t('botChallenge.captchaAssets.saveSuccess')); void queryClient.invalidateQueries({ queryKey: ['captcha-assets-config'] }); },
    onError: (error) => toast.error(errorMessage(error, t('botChallenge.captchaAssets.saveFailed'))),
  });
  const testMutation = useMutation({
    mutationFn: testCAPTCHAAssetConfig,
    onSuccess: () => toast.success(t('botChallenge.captchaAssets.testSuccess')),
    onError: (error) => toast.error(errorMessage(error, t('botChallenge.captchaAssets.testFailed'))),
  });
  ...
  return <div className={styles.panel}>
    ...
    {forbidden ? <State icon={<ShieldX/>} title={t('botChallenge.captchaAssets.forbidden')} hint={t('botChallenge.captchaAssets.forbiddenHint')}/> : section === 'assets' ? <>
      <div className={styles.toolbar}>
        <Select value={kind} onValueChange={(value) => setKind(value as CAPTCHAAssetKind | 'all')}>
          <SelectTrigger aria-label={t('botChallenge.captchaAssets.filter')} className="w-[160px]">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">{t('botChallenge.captchaAssets.allTypes')}</SelectItem>
            {KINDS.map((item) => <SelectItem key={item} value={item}>{t(`botChallenge.captchaAssets.kinds.${item}`)}</SelectItem>)}
          </SelectContent>
        </Select>
        <div className={styles.uploadControls}>
          <Select value={uploadKind} onValueChange={(value) => setUploadKind(value as CAPTCHAAssetKind)}>
            <SelectTrigger aria-label={t('botChallenge.captchaAssets.uploadType')} className="w-[160px]">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {KINDS.map((item) => <SelectItem key={item} value={item}>{t(`botChallenge.captchaAssets.kinds.${item}`)}</SelectItem>)}
    ...
    <Dialog open={Boolean(preview)} onOpenChange={(open) => {
      ...
      <DialogContent>
        ...
        {preview && <div className={styles.preview}><img src={preview.url} alt={preview.asset.name}/><span>{t('botChallenge.captchaAssets.previewOneTime')}</span></div>}
      </DialogContent>
    </Dialog>
    ...
    <Dialog open={Boolean(deleteTarget)} onOpenChange={(open) => { if (!open) setDeleteTarget(null); }}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t('botChallenge.captchaAssets.deleteTitle')}</DialogTitle>
          <DialogDescription>
            {t('botChallenge.captchaAssets.deleteConfirm', { name: deleteTarget?.name ?? '' })}
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
  ...
  return (
    <div className={styles.configArea}>
      <div className={styles.configHeading}>
        <div>
          <h2>{t('botChallenge.captchaAssets.storageConfig')}</h2>
          <p>{t('botChallenge.captchaAssets.storageHint')}</p>
        </div>
        <div>
          <Button variant="outline" loading={testing} onClick={onTest}><Cloud size={16}/>{t('botChallenge.captchaAssets.test')}</Button>
          <Button loading={saving} onClick={onSave}><Save size={16}/>{t('common.save')}</Button>
        </div>
      </div>
      <div className={styles.backendSwitch}>
        <button type="button" className={config.backend === 'local' ? styles.activeBackend : undefined} onClick={() => patch({ backend: 'local' })}>
          <HardDrive size={18}/><span><strong>{t('botChallenge.captchaAssets.local')}</strong><small>{t('botChallenge.captchaAssets.localHint')}</small></span>
        </button>
        <button type="button" className={config.backend === 's3' ? styles.activeBackend : undefined} onClick={() => patch({ backend: 's3' })}>
          <Cloud size={18}/><span><strong>S3</strong><small>{t('botChallenge.captchaAssets.s3Hint')}</small></span>
        </button>
      </div>
      {config.backend === 'local' ? (
        <div className={styles.formGrid}>
          <Field label={t('botChallenge.captchaAssets.localPath')}>
            <Input value={config.local.path} onChange={(e) => patch({ local: { path: e.target.value } })}/>
          </Field>
        </div>
      ) : (
        <div className={styles.formGrid}>
          <Field label={t('botChallenge.captchaAssets.endpoint')}>
            <Input value={config.s3.endpoint} onChange={(e) => patchS3({ endpoint: e.target.value })}/>
          </Field>
          <Field label={t('botChallenge.captchaAssets.bucket')}>
            <Input value={config.s3.bucket} onChange={(e) => patchS3({ bucket: e.target.value })}/>
          </Field>
          <Field label={t('botChallenge.captchaAssets.region')}>
            <Input value={config.s3.region} onChange={(e) => patchS3({ region: e.target.value })}/>
          </Field>
          <Field label={t('botChallenge.captchaAssets.prefix')}>
            <Input value={config.s3.prefix} onChange={(e) => patchS3({ prefix: e.target.value })}/>
          </Field>
          <Field label={t('botChallenge.captchaAssets.credential')} hint={credentialConfigured ? t('botChallenge.captchaAssets.credentialConfigured') : t('botChallenge.captchaAssets.credentialMissing')}>
            <Input type="password" autoComplete="new-password" value={config.s3.credential_file} placeholder={credentialConfigured ? '••••••••' : ''} onChange={(e) => patchS3({ credential_file: e.target.value })}/>
          </Field>
          <Field label={t('botChallenge.captchaAssets.metadataKey')} hint={metadataKeyConfigured ? t('botChallenge.captchaAssets.metadataKeyConfigured') : t('botChallenge.captchaAssets.metadataKeyMissing')}>
            <Input type="password" autoComplete="new-password" value={config.s3.metadata_key_file} placeholder={metadataKeyConfigured ? '••••••••' : ''} onChange={(e) => patchS3({ metadata_key_file: e.target.value })}/>
          </Field>
          <Field label={t('botChallenge.captchaAssets.timeout')}>
            <Input type="number" min={1} value={String(config.s3.request_timeout / NANOSECONDS_PER_SECOND)} onChange={(e) => patchS3({ request_timeout: Math.max(1, Number(e.target.value) || 1) * NANOSECONDS_PER_SECOND })}/>
          </Field>
          <Toggle label={t('botChallenge.captchaAssets.useTLS')} checked={config.s3.use_tls} onChange={(use_tls) => patchS3({ use_tls })}/>
          <Toggle label={t('botChallenge.captchaAssets.pathStyle')} checked={config.s3.path_style} onChange={(path_style) => patchS3({ path_style })}/>
          <div className={styles.privateEndpoint}>
            <Toggle label={t('botChallenge.captchaAssets.allowPrivateEndpoint', { defaultValue: 'Allow private or local S3 endpoints' })} checked={config.s3.allow_private_endpoint} onChange={(allow_private_endpoint) => patchS3({ allow_private_endpoint })}/>
            {config.s3.allow_private_endpoint && (
              <p className={styles.riskHint} role="alert">
                <ShieldAlert size={17}/>
                <span>{t('botChallenge.captchaAssets.allowPrivateEndpointRisk', { defaultValue: 'Private endpoint access lets the server connect to local, private, and link-local addresses. Enable it only for a trusted S3 endpoint.' })}</span>
              </p>
            )}
          </div>
        </div>
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (value: boolean) => void }) { return <label className={styles.toggle}><span>{label}</span><Switch checked={checked} onCheckedChange={onChange}/></label>; }
...
function AssetSkeleton() { return <div className={styles.skeleton}><Skeleton className="h-32 w-full" /></div>; }
