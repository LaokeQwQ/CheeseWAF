...
export default function CaptchaAssetsPanel() {
  ...
  return <div className={styles.panel}>
    ...
    </> : configQuery.isLoading ? <AssetSkeleton/> : configQuery.isError ? <State icon={<ShieldX/>} title={t('botChallenge.captchaAssets.configLoadFailed')} hint={errorMessage(configQuery.error, '')} action={<Button onClick={() => configQuery.refetch()}>{t('common.retry')}</Button>}/> : !config ? <Empty description={t('botChallenge.captchaAssets.empty')}/> : <ConfigEditor config={config} setConfig={setConfig} credentialConfigured={credentialConfigured} metadataKeyConfigured={metadataKeyConfigured} saving={saveMutation.isPending} testing={testMutation.isPending} onSave={() => saveMutation.mutate(config)} onTest={() => testMutation.mutate(config)}/>}
