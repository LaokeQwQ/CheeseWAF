...
function readLogsPermission(): EventPermission {
  try {
    ...
    if (cached) {
      ...
      if (account.role === 'admin') return 'allowed';
      if (account.role === 'readonly') return 'allowed'; // read:logs typically granted
      if (account.role) return 'unknown';
    }
  } catch {
    /* fall through */
  }
  // Without JWT body scopes, treat as unknown (UI will probe API).
  return 'unknown';
}
