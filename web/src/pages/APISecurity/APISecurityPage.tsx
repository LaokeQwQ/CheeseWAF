        ...
        <section className="panel api-discovery-panel">
          ...
          <div className="table-scroll api-endpoints-table">
            <Table rowKey={(r) => r.method + r.path} pagination={false} data={visibleEndpoints}
              columns={[
                { title: t('apisec.method'), dataIndex: 'method', width: 88, render: (v: string) => <Tag>{v}</Tag> },
                { title: t('apisec.path'), dataIndex: 'path', width: 180, render: (v: string) => <code className="table-code" title={v}>{v}</code> },
                { title: t('apisec.count'), dataIndex: 'count', width: 92 },
                { title: t('apisec.blocked'), dataIndex: 'blocked', width: 86, render: (v: number) => <Tag color={v > 0 ? 'red' : 'green'}>{v}</Tag> },
                { title: t('apisec.status'), dataIndex: 'status_family', width: 240,
