...
export interface RestoreSliderChallengeData { image:string; width?:number; height?:number; movingPart?:'top'|'bottom'; maxOffsetPercent?:number; initialOffsetPercent?:number; minDurationMs?:number }
...
export function RestoreSliderChallenge({challenge,disabled,label,onChange,onComplete}:RestoreSliderChallengeProps){
  ...
  const offset=restoreOffsetForSlider(initial,max,value); const answer=():RestoreSliderAnswer=>{const durationMs=Math.max(elapsed(started.current),challenge.minDurationMs??0);const point={x:value,y:5000};return{offset:Math.round(offset*100)/100,point,track:appendTrack(operation.current,trackPoint(point,durationMs,'up')),durationMs}};
  ...
  return <div className={styles.challenge} data-testid="restore-slider-challenge">
    ...
    <div className={styles.controls}><span className={styles.dragIcon} aria-hidden="true"><MoveRight size={25}/></span><input className={styles.range} aria-label={resolvedLabel} type="range" min={0} max={10000} step={1} value={value} disabled={disabled} style={{'--captcha-progress':`${value/100}%`} as CSSProperties} onPointerDown={event=>{pointer.current=event.pointerId;started.current=performance.now();operation.current=[trackPoint({x:value,y:5000},0,'down')];event.currentTarget.setPointerCapture?.(event.pointerId)}} onChange={event=>{const next=Number(event.currentTarget.value);setValue(next);const nextOffset=restoreOffsetForSlider(initial,max,next);const point={x:next,y:5000};const durationMs=Math.max(elapsed(started.current),challenge.minDurationMs??0);operation.current=appendTrack(operation.current,trackPoint(point,durationMs,'move'));onChange?.({offset:Math.round(nextOffset*100)/100,point,track:operation.current,durationMs})}} onPointerUp={finish} onPointerCancel={()=>{pointer.current=undefined;operation.current=[]}} onKeyDown={event=>{if(!event.repeat&&['ArrowLeft','ArrowRight','Home','End','PageUp','PageDown'].includes(event.key)){started.current=performance.now();operation.current=[trackPoint({x:value,y:5000},0,'down')];}}} onKeyUp={event=>{if(['ArrowLeft','ArrowRight','Home','End','PageUp','PageDown'].includes(event.key))finish()}}/></div>
  </div>
}
