...
export function AngleChallenge({ challenge, disabled, label, onChange, onComplete }: AngleChallengeProps) {
  ...
  const finish = (event: ReactPointerEvent<HTMLInputElement>) => { if (pointer.current !== event.pointerId) return; const durationMs = completedDuration(); operation.current = appendTrack(operation.current, trackPoint({ x: value, y: 5000 }, durationMs, 'up'), DEFAULT_TRACK_LIMIT); pointer.current = undefined; onComplete({ ...current(), durationMs }); };
