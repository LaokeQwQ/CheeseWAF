...
export function stripInternalProcessPhrases(value: string) {
  const cleaned = value
    ...
    .replace(/真实工具\s*observation/gi, '安全事件数据')
    .replace(/工具\s*observation/gi, '安全事件数据')
    .replace(/\btool\s+observations?\b/gi, 'security event data')
    .replace(/\bobservations?\b/gi, 'security event data')
