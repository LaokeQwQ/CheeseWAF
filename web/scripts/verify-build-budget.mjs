...
const themeAssets = cssAssets.filter((file) => /^(light|dark|black-gold|blue-white)-/.test(file));
...
if (themeAssets.length !== 4) {
  throw new Error(`Expected four independently loadable theme stylesheets, found ${themeAssets.length}`);
}
...
console.log(
  `Build budgets OK: ${preloadPaths.length} initial preloads / ${(preloadBytes / 1024).toFixed(2)} KiB gzip; ${(initialCSSWithThemeBytes / 1024).toFixed(2)} KiB initial CSS gzip including the largest theme; four lazy theme stylesheets.`,
);
