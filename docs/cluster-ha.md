...
M2 foundations do not yet mean a node can fully join a running multi-node control plane. The basic Web deployment wizard foundation is available, and the single-node SSH `install` / `rollback-install` actions can now perform a binary install/update and restore the newest binary backup, but the full Web installer wizard, automatic join orchestration, rolling upgrade, `monitor-node` runtime, heartbeat, majority confirmation, Raft/etcd coordination, object reconciliation, protection-mode write freezing, and production traffic scheduling are still follow-up work. The current audit view is a management/deployment record view, not a substitute for M3 runtime consensus or object reconciliation audit.
