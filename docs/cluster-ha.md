...
- Cluster identity state with a persistent cluster CA, registered node metadata, and CSR-signed node certificates suitable for later mTLS wiring.
- Runtime heartbeat foundations are available through mTLS-bound `POST /api/cluster/nodes/{id}/heartbeat`, status aggregation, and Web node runtime state display. The heartbeat handler requires a verified node client certificate whose enrolled serial number and node identity match the URL node ID; ordinary management sessions or API tokens cannot forge node liveness. The status calculator now uses configured nodes plus recent write-capable heartbeats to compute online node count, voting node count, majority confirmation, and basic protection-mode write freezing. Heartbeat writes are kept out of the operator audit feed to avoid turning routine liveness traffic into noisy management actions.

M2 foundations do not yet mean a node can fully join a running multi-node control plane. The basic Web deployment wizard foundation is available, and the single-node SSH `install` / `rollback-install` actions can now perform a binary install/update and restore the newest binary backup. Runtime heartbeat and basic majority/write-freeze status aggregation have started, but the full Web installer wizard, automatic join orchestration, rolling upgrade, `monitor-node` runtime, multi-link heartbeat, Raft/etcd coordination, object reconciliation, and production traffic scheduling are still follow-up work. The current audit view is a management/deployment record view, not a substitute for M3 runtime consensus or object reconciliation audit.
...
Backend, CLI, and Web foundations are advancing, but M2 as a whole is not complete. This milestone now has deployment package generation, temporary SSH deployment checks/fixed actions, a single-node SSH binary install/update path, a newest-backup binary restore action, asynchronous deployment tasks with a basic audit timeline, a cluster audit page/API backed by management audit and deployment task events, one-time join tokens, token-authenticated CSR-based node enrollment, persistent cluster CA, local-key certificate issuance, CSR-based certificate rotation foundations, local CLI bootstrap, Web token/node/task/certificate signing views, a basic Web deployment wizard for Ansible package generation and temporary SSH checks/actions, and token/node revocation primitives. The full guided Web installer wizard, automatic join orchestration, rolling upgrade, `monitor-node` runtime, heartbeat, majority confirmation, Raft/etcd coordination, object reconciliation, and production traffic scheduling still need follow-up work before this can be presented as a complete cluster expansion experience.

### M3: Consistency And Protection Mode

Planned. This milestone will add node interconnect, multi-path heartbeat, built-in consistency service, optional external etcd, majority confirmation, write freezing in protection mode, and cluster-aware health decisions.
...
- Add representative attack samples and representative false-positive samples.
...
- Complete the AI assistant's built-in WAF knowledge base, prompt hardening, long reasoning stream keepalive, and streaming single-event analysis trace.
