...
- A basic Web deployment wizard foundation is available for Ansible package generation and temporary SSH deployment checks/actions. It includes method selection, cluster/channel/node inputs, package preview/copy/download, SSH authentication and host-key verification choices, precheck gating, task timeline/cards, and mobile layout hardening. This is still a foundation, not the production-grade installer wizard.
