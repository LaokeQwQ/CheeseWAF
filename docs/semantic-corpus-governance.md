...
- 已验证：2026-09-01 对公开来源做了只读准入审计；[SR-BH 2020](open-corpus-catalog.md) 具备公开版本和 CC0 记录但仍有单一 WordPress 环境、WAF 派生标签及 PII/Cookie/Token 清洗风险，[ModSec-WP](open-corpus-catalog.md) 当前未确认文件级许可证且同样是单一测试床，[openappsec 良性包](open-corpus-catalog.md) 没有独立攻击标签；Biblio-US17 需按资源页申请且仅公开 URI 粒度，已明确排除；一个公开页面标注 CC BY 4.0 的 SQLi 候选因文件/字段/哈希证据不足仍留在隔离区。以上来源均未下载完整副本、未进入 formal/blind、未改变任何质量分母；H23Q/AWID、第三方合并仓库和需要申请/机构认证的来源已明确排除。
...
- 待完成：用独立且已锁定的开放来源生成正式 blind 快照，并在真实站点/时间切片上持续跑性能基准。当前仍没有同时满足文件级许可证、精确版本、完整请求上下文、独立标签、去敏和跨组独立性的开放原始请求；现有 payload-only、批量派生和本地合成文件不能充当独立 blind。取得合法来源或在授权靶场生成流量后，仍须走同一套全局去重、初筛、语义挑选、脱敏、分组切分和首次锁定流程；在此之前不生成虚假的独立泛化指标。
