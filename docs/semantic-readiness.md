...
- Additional SQLi handling for error-based XML functions (`extractvalue`/`updatexml`-style patterns), database time-delay functions including `pg_sleep`, boolean predicates that use SQL string functions such as `char()`, hex tautologies, `ORDER BY` enumeration, SQL Server `xp_cmdshell`, and database file read/write primitives such as `load_file` / `into outfile`, without flagging standalone documentation text or SQL comment tutorials.
...
- SSRF handling for loopback/cloud metadata hosts, dotted-decimal, dotted-hex, dotted-octal, IPv6 loopback, IPv4-mapped IPv6 loopback, and local `file://` URL schemes in fetch-sink contexts.
...
- `TestAnalyzerReadinessMatrix` covers common SQLi/XSS/RCE/LFI/XXE/SSRF/NoSQLi/SSTI payloads, cookie and multipart inputs, SQL comment keyword splitting, MySQL versioned comments, database file-read/write and error-based function side effects, PostgreSQL time delay functions, SQL Server command-execution primitives, function-based boolean SQLi, Unicode/control-character/entity/data-URI/srcdoc/formaction/srcset XSS, `${IFS}` command injection, PowerShell/Pwsh encoded or dynamic execution, inline interpreter execution, Windows traversal, sensitive local file targets, internal-network SSRF including decimal/hex/octal/IPv6/IP-mapped IPv6/file-scheme forms, MongoDB operator injection in JSON/form login or query structures, and template-injection execution chains.
