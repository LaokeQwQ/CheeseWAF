...
当前硬化重点包括基于公开 corpus 思路精选的语义样本、真实仪表盘计数、实时/总计态势分离、Dashboard 图表 scoped 尺寸修复、真实主机 CPU/load/内存/Swap/磁盘资源指标、资源回收操作、服务端解析真实日志的单事件详情与 AI 分析、可通过 URL 定位的 IP 威胁情报/访问名单页签、全局/站点/目录级 IP allow/block 规则、可信代理/CDN 真实客户端 IP 解析、手动 IP 信誉分覆盖、真实健康重连状态、更清晰的 2D/中国区域/3D 攻击地图模式、APISec JWT 签名/audience/远端 JWKS/端点策略控制、路由级管理 API RBAC、可投递的日报/周报安全报告、GitHub 三分支渠道 artifact，以及服务端单事件 AI 分析 lookup。已晋级的 `master` 快照 `29f27264` 已以 `0.1.0-beta.147+29f27264b95c` 部署到远端验收主机并完成 smoke 测试：管理端 health/index 返回 200，代理首页返回 200，SQLi probe 返回 403，部署态 HTTP corpus 40/40 通过（30/30 攻击拦截、10/10 benign 未拦截），HTTPS 管理端响应包含 frame、nosniff、referrer、permissions 与 HSTS 安全头。下一步发行工作需要修复 Forgejo mirror freshness，并补齐外部动态扫描证据，再标记 V0.1 beta。
