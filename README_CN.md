...
- 前端构建输出使用稳定的 Vite/Rolldown vendor chunks，将 React、Appica、Three.js、可视化、运行时和 UI 工具依赖分组。生产构建默认不输出 source map，只有显式设置 `VITE_SOURCEMAP=true` 才保留调试 map，从而减小发布版 Web 控制台体积；较大的 Three.js 依赖只在攻击地图路径按需加载。
