...
当前硬化重点包括基于公开 corpus 思路精选的语义样本、真实仪表盘计数、实时/总计态势分离、Dashboard 图表 scoped 尺寸修复、真实主机 CPU/load/内存/Swap/磁盘资源指标、资源回收操作、单事件日志详情与 AI 分析、可通过 URL 定位的 IP 威胁情报页签、真实健康重连状态、更清晰的 2D/中国大陆/3D 攻击地图模式、APISec JWT 签名/audience/远端 JWKS/端点策略控制、路由级管理 API RBAC，以及 GitHub/Forgejo 为 `dev`、`canary`、`stable` 三个渠道同步生成分支 artifact。最近的 GitHub `canary` 与 `master` push runs（`27135688931`、`27136147773`、`27136628202`）均已通过 Go 多平台测试、Web build、cross-build，并在适用分支上传 `release-artifacts`。代码快照 `e3a8b80` 已构建为 Linux amd64 单二进制部署，并在远端验收主机 smoke 测试：管理端 health/index 返回 200，代理首页返回 200，SQLi probe 返回 403，HTTPS 管理端响应包含 frame、nosniff、referrer、permissions 与 HSTS 安全头。已晋级的硬化快照 `df244ca` 已完成打包；远端验收机可用时，下一步应重新部署新的 master 快照。
