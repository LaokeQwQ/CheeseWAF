...
- 前端构建输出使用稳定的 Vite/Rolldown vendor chunks，将 React、Radix/shadcn、Three.js、可视化、运行时和 UI 工具依赖分组。生产构建默认不输出 source map，只有显式设置 `VITE_SOURCEMAP=true` 才保留调试 map，从而减小发布版 Web 控制台体积；较大的 Three.js 依赖只在攻击地图路径按需加载。
...
- IP 访问控制现在支持全局、站点级、目录/路径级 allow/block 规则，并兼容旧的全局白名单/黑名单。allow 规则沿用现有白名单对 IP 相关防护的旁路语义，block 规则会在转发到源站前拦截；IP 画像可手动覆盖 0-100 信誉分。泛化 `trusted_cidrs` 只采信经过可信链校验的 `X-Forwarded-For` 和 RFC `Forwarded`；`CF-Connecting-IP`、`True-Client-IP`、`Fastly-Client-IP`、`Fly-Client-IP`、`DO-Connecting-IP`、`Ali-CDN-Real-IP`、`X-Azure-ClientIP`、`X-Vercel-Forwarded-For` 等供应商专用头必须在 `trusted_proxy_providers` 中显式绑定对应供应商 CIDR，避免其它可信中间代理伪造 CDN 身份头。
