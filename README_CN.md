...
- 反向代理 WAF 主流程：分阶段语义分析（输入提取、深度解码、词法/语法/行为评分）、自定义规则、IP/ACL/限流/Bot 防护、威胁情报导入与订阅、签名 JS 工作量证明挑战、Altcha 风格 PoW CAPTCHA、带 opaque token 服务端音频与尝试次数限制的图像验证码、拼图滑块验证码、排队室、边缘缓存/请求头/压缩策略，以及响应体检测。
