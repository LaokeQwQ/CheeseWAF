...
- 管理端 token 带唯一 token ID，并由服务端可撤销 session 支撑。登录会创建 session，`/api/auth/refresh` 会原子撤销旧 token ID 并签发新 token，`/api/auth/logout` 会在 Web 控制台清理本地状态前撤销当前 session。控制台会在请求前自动刷新仍有效但接近过期的 token；过期或无效 token 仍走正常 401 登出流程。
