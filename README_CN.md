...
截至 2026-06-09，最新发行链批次已在 GitHub 完成受保护的向上晋级：PR #55 合并 `feature/server-side-event-ai-analysis -> dev`，PR #56 晋级 `dev -> canary`，PR #57 晋级 `canary -> master`。`git.laoker.cc/Laoke/CheeseWAF` 上的 Forgejo 是计划中的主要 forge/构建目标；当前 GitHub 仍作为 pull mirror 的源仓库和辅助 CI 检查。GitHub Actions master push run `27217637012` 已在 `29f27264` 上通过并生成 release artifacts。GitHub 合并完成后已触发 Forgejo mirror-sync，但本次检查时 Forgejo mirror 没有追上最新快照，仍停在上一批 `dev=00f7912`、`canary=e3d692e`、`master=e55c578`，因此当前 Forgejo 状态被记录为镜像同步异常，不能作为最新发行通过证据。Forgejo workflow 位于 `.forgejo/workflows/ci.yml`，并使用 `scripts/ci/setup-go-mirror.sh` 和 `scripts/ci/setup-node-mirror.sh` 支持自托管 runner 友好的工具链设置。
