...
截至 2026-06-09，最新发行链批次已在 GitHub 完成受保护的向上晋级：PR #52 合并 `feature/security-report-productization -> dev`，PR #53 晋级 `dev -> canary`，PR #54 晋级 `canary -> master`。`git.laoker.cc/Laoke/CheeseWAF` 上的 Forgejo 是主要 forge/构建目标，GitHub 保持辅助镜像/检查角色；GitHub 合并完成后已触发 Forgejo mirror-sync，并在该快照上与 GitHub 对应分支 head 一致：`dev`（`00f7912`）、`canary`（`e3d692e`）、`master`（`e55c578`）。GitHub Actions master push run `27212626546` 已通过并生成 release artifacts；Forgejo refs 已同步，但最新 Forgejo Actions runs 在本快照时仍处于 queued/running，不能宣称 Forgejo CI 已通过。Forgejo workflow 位于 `.forgejo/workflows/ci.yml`，并使用 `scripts/ci/setup-go-mirror.sh` 和 `scripts/ci/setup-node-mirror.sh` 支持自托管 runner 友好的工具链设置。

当前硬化重点包括基于公开 corpus 思路精选的语义样本、真实仪表盘计数、实时/总计态势分离、Dashboard 图表 scoped 尺寸修复、真实主机 CPU/load/内存/Swap/磁盘资源指标、资源回收操作、服务端解析真实日志的单事件详情与 AI 分析、可通过 URL 定位的 IP 威胁情报/访问名单页签、全局/站点/目录级 IP allow/block 规则、可信代理/CDN 真实客户端 IP 解析、手动 IP 信誉分覆盖、真实健康重连状态、更清晰的 2D/中国大陆/3D 攻击地图模式、APISec JWT 签名/audience/远端 JWKS/端点策略控制、路由级管理 API RBAC、可投递的日报/周报安全报告，以及 GitHub/Forgejo 为 `dev`、`canary`、`stable` 三个渠道同步生成分支 artifact。已晋级的 `master` 快照 `e55c578` 已部署到远端验收主机并完成 smoke 测试：管理端 health/index 返回 200，代理首页返回 200，SQLi probe 返回 403，部署态 HTTP corpus 40/40 通过（30/30 攻击拦截、10/10 benign 未拦截），HTTPS 管理端响应包含 frame、nosniff、referrer、permissions 与 HSTS 安全头。下一步发行工作需要补齐外部动态扫描证据，并继续跟踪 Forgejo runner 队列，再标记 V0.1 beta。
