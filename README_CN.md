...
截至 2026-06-09，最新硬化发行链批次已在 GitHub 完成受保护的向上晋级：PR #30 合并 `fix/admin-health-ai-events -> dev`，PR #31 晋级 `dev -> canary`，PR #32 晋级 `canary -> master`。`git.laoker.cc/Laoke/CheeseWAF` 上的 Forgejo 是主要 forge/构建目标，GitHub 保持辅助镜像/检查角色；GitHub 合并完成后已触发 Forgejo mirror-sync，并在该快照上与 GitHub 对应分支 head 一致：`dev`（`081416a`）、`canary`（`1f8eab1`）、`master`（`9719b01`）。这三个受保护分支当时的代码树内容一致，同时保留各自按规则向上 PR 产生的 merge commit。Forgejo workflow 位于 `.forgejo/workflows/ci.yml`，并使用 `scripts/ci/setup-go-mirror.sh` 和 `scripts/ci/setup-node-mirror.sh` 支持自托管 runner 友好的工具链设置。

当前硬化重点包括基于公开 corpus 思路精选的语义样本、真实仪表盘计数、实时/总计态势分离、Dashboard 图表 scoped 尺寸修复、真实主机 CPU/load/内存/Swap/磁盘资源指标、资源回收操作、单事件日志详情与 AI 分析、可通过 URL 定位的 IP 威胁情报/访问名单页签、全局/站点/目录级 IP allow/block 规则、可信代理/CDN 真实客户端 IP 解析、手动 IP 信誉分覆盖、真实健康重连状态、更清晰的 2D/中国大陆/3D 攻击地图模式、APISec JWT 签名/audience/远端 JWKS/端点策略控制、路由级管理 API RBAC，以及 GitHub/Forgejo 为 `dev`、`canary`、`stable` 三个渠道同步生成分支 artifact。已晋级的 `master` 快照 `9719b01` 已部署到远端验收主机并完成 smoke 测试：管理端 health/index 返回 200，代理首页返回 200，SQLi probe 返回 403，HTTPS 管理端响应包含 frame、nosniff、referrer、permissions 与 HSTS 安全头。下一步发行工作需要补齐已部署 WAF 的可重复安全回放和动态扫描证据，再标记 V0.1 beta。
