...
- 管理端登录使用可配置的控制台登录配置：Altcha 兼容 PoW 验证码默认开启，通过 `/api/auth/captcha` 下发挑战，登录提交必须先通过服务端校验再进入密码/2FA 检查；系统设置可开关登录验证码，并调整有效期与难度。登录页不再提示默认用户名/密码，登录卡片下方以弱提示显示加载耗时，并支持通过 `console.login.background` 配置图片或动态视频 URL 作为自定义背景。
