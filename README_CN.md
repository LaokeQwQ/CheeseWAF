...
- 拦截/报错页已从“只读预览”升级为运行时真实配置：代理默认渲染更完整的内置 HTML 模板，每个拦截或代理错误响应都会在页面正文、`X-CheeseWAF-Trace-ID` 响应头和 WAF 日志事件中显示同一个事件 / Trace ID；Web 控制台可启用内置模板、上传或编辑自定义 Go `html/template` HTML，后端校验语法后持久化到 YAML 并热更新代理渲染器，无需重启。管理 API 错误和前端运行时错误也会显示 Trace ID，便于和后端日志、浏览器控制台对齐排查。
