...
- 拦截/报错页已从“只读预览”升级为运行时真实配置：代理默认渲染正式的基础设施风格内置 HTML 模板，包含内联 CheeseWAF logo、客户端/WAF/源站状态流、运维排障提示和可见 Event / Trace ID。每个拦截或代理错误响应都会在页面正文、`X-CheeseWAF-Trace-ID`、`X-CheeseWAF-Event-ID`、`Cache-Control: no-store` 和 WAF 日志事件中显示同一个排障 ID；Web 控制台可启用内置模板、上传或编辑自定义 Go `html/template` HTML，后端校验语法后持久化到 YAML 并热更新代理渲染器，无需重启。如果自定义 HTML 漏写 Event / Trace ID，CheeseWAF 会自动注入可见兜底角标。管理 API 错误会返回 Trace ID，已登录态前端运行时错误会用页面 ErrorBoundary 显示的同一个 `cw-ui-*` ID 上报到后端日志 sink，便于把浏览器报错和持久化日志对齐排查。
...
截至 2026-06-09，最新发行链批次已在 GitHub 完成受保护的向上晋级：PR #55 合并 `feature/server-side-event-ai-analysis -> dev`，PR #56 晋级 `dev -> canary`，PR #57 晋级 `canary -> master`。`git.laoker.cc/Laoke/CheeseWAF` 上的 Forgejo 是计划中的主要 forge/构建目标；当前 GitHub 仍作为 pull mirror 的源仓库和辅助 CI 检查。GitHub Actions master push run `27217637012` 已在 `29f27264` 上通过并生成 release artifacts；后续 README 快照文档也通过 PR #58/#59/#60 向上合并，并由 master push run `27220251726` 通过。GitHub 合并完成后已触发 Forgejo mirror-sync，当前 Forgejo refs 已追上分支 head：`dev=230b3db`、`canary=a99dee0`、`master=2c5acd6`。本快照时这些 ref 对应的 Forgejo Actions 仍在 running/waiting 队列中，因此最新绿色构建证据仍以 GitHub 侧为准，Forgejo 结果需继续跟踪。Forgejo workflow 位于 `.forgejo/workflows/ci.yml`，并使用 `scripts/ci/setup-go-mirror.sh` 和 `scripts/ci/setup-node-mirror.sh` 支持自托管 runner 友好的工具链设置。

当前硬化重点包括基于公开 corpus 思路精选的语义样本、真实仪表盘计数、实时/总计态势分离、Dashboard 图表 scoped 尺寸修复、真实主机 CPU/load/内存/Swap/磁盘资源指标、资源回收操作、服务端解析真实日志的单事件详情与 AI 分析、可通过 URL 定位的 IP 威胁情报/访问名单页签、全局/站点/目录级 IP allow/block 规则、可信代理/CDN 真实客户端 IP 解析、手动 IP 信誉分覆盖、真实健康重连状态、更清晰的 2D/中国大陆/3D 攻击地图模式、APISec JWT 签名/audience/远端 JWKS/端点策略控制、路由级管理 API RBAC、可投递的日报/周报安全报告、GitHub 三分支渠道 artifact，以及服务端单事件 AI 分析 lookup。已晋级的 `master` 快照 `29f27264` 已以 `0.1.0-beta.147+29f27264b95c` 部署到远端验收主机并完成 smoke 测试：管理端 health/index 返回 200，代理首页返回 200，SQLi probe 返回 403，部署态 HTTP corpus 40/40 通过（30/30 攻击拦截、10/10 benign 未拦截），HTTPS 管理端响应包含 frame、nosniff、referrer、permissions 与 HSTS 安全头。下一步发行工作需要修复 Forgejo mirror freshness，并补齐外部动态扫描证据，再标记 V0.1 beta。
