...
- Block/error pages are now runtime-configurable instead of preview-only. The proxy renders a polished built-in HTML template by default, every block or proxy error response exposes the same event/Trace ID in the page body, `X-CheeseWAF-Trace-ID` response header, and WAF log entry, and the Web console can enable built-in templates, upload/admin-edit custom Go `html/template` HTML, validate it server-side, persist it to YAML, and hot-reload the proxy renderer without a restart. Management API errors return Trace IDs, and authenticated frontend runtime errors are reported into the backend log sink with the same `cw-ui-*` ID shown by the error boundary for cross-checking browser reports against stored logs.
