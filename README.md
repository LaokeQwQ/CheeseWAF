...
| **Cluster Plane** | `https://127.0.0.1:9444` | TLS/mTLS node interconnect for health, heartbeats, topology, and orchestration; not a site/policy replication channel |
...
The default admin listener is `127.0.0.1:9443`. On the server (or over an SSH tunnel) open `http://127.0.0.1:9443/setup`. Every setup mutation, including loopback requests, requires `X-CheeseWAF-Setup-Token`; use the token from the startup log or the protected `/var/lib/cheesewaf/setup.url` fragment (or set `CHEESEWAF_SETUP_TOKEN`). From another host, the remote `SERVER_IP:9443` stays closed until setup explicitly chooses a public admin strategy.
