...
- Admin login now uses a configurable console login profile: an Altcha-compatible proof-of-work CAPTCHA is enabled by default through `/api/auth/captcha`, login submissions must pass the server-verified challenge before password/2FA checks, and System Settings can toggle the login CAPTCHA plus tune its TTL/difficulty. The login page no longer hints default credentials, shows unobtrusive load timing in the lower-left corner, and supports operator-defined image or video background URLs through `console.login.background`.
