...
3. **Dynamic Rule Synthesis:** When enabled for a site, high-confidence malicious findings (`high` or `critical`) generated by the model can be promoted into a persistent, site-scoped payload custom rule applied to the data plane. IP and fingerprint blocks remain explicit operator decisions.

The current runnable management-storage profile for the main `cheesewaf serve` process is `storage.profile: temporary`, which stores management state in embedded SQLite. `storage.profile: production` still fails closed with `ErrProductionStorageUnavailable` until the complete management-store, control-plane, Redis, approval, and data-plane lifecycle is wired into one service startup. An independent `cheesewaf-control` binary now provides a loopback status/health process backed by the control-plane PostgreSQL tables and native-raft adapter; it does not by itself make the main WAF production profile ready. `storage.postgresql` is only an optional asynchronous access-log sink; Redis is not wired as the Bot challenge backend. Cluster coordination in the main WAF remains the builtin single-node path; selecting `etcd` stays fail-closed because no etcd coordinator is shipped. `internal/crp` provides offline verification, local staged state, and a sidecar activation service, but no main-WAF CLI/store/OTA lifecycle is mounted yet. `internal/cwedp` has a pull transport adapter, while endpoint registration, durable leases, audit and cluster orchestration remain integration work. `cheesewaf temporary-online probe` is an explicit local, one-shot HTTPS broker with administrator password confirmation, direct-IP TLS pinning, and durable metadata audit; it does not start from `serve` and is not yet a CWEDP or control-plane integration.
...
- **ALAP Asynchronous Auditing**: Uses the configured OpenAI- or Anthropic-compatible provider in background worker queues without adding latency to live HTTP traffic; private/local endpoints require the explicit `allow_private_api_base` setting.
...
- **Access Control & Bot Mitigation**: Built-in IP allow/deny lists, GeoIP blocking, client soft fingerprinting, slider CAPTCHA challenges, sharded sliding-window rate limiting, and waiting rooms.
...
- **Current storage boundary**: `storage.profile: temporary` uses SQLite for the main WAF management state. The main WAF rejects `storage.profile: production` until its full startup unit is connected. The standalone `cheesewaf-control` process can validate and serve control-plane health/status with PostgreSQL control state and native-raft, but it is not a replacement for the main management store. `storage.postgresql` remains an optional log sink; Redis-backed Bot challenge storage is rejected by configuration validation. The builtin cluster path is single-node only; configured etcd remains fail-closed.

---

## Request Processing Pipeline

Solid lines denote the inline millisecond data plane; dashed lines represent post-response asynchronous ALAP auditing and site custom-rule reloads:
...
| **Cluster Plane** | `https://127.0.0.1:9444` | Optional TLS/mTLS node interconnect when `cluster.enabled: true`; health, heartbeats, topology, and orchestration hooks only, not site/policy replication |
...
> **Notes:**
> 1. **Dynamic Elevation (`promote_seconds`)**: Under Level 4, detecting embedded attack patterns can trigger a temporary elevation to Level 5 for a specified window (e.g., 300 seconds). The current runtime persists the deadline in SQLite.
...
CheeseWAF provides four independent deployment methods. Choose the one that suits your infrastructure:
...
# Copy the tracked template to the dedicated runtime path consumed by systemd.
# Edit /etc/cheesewaf/cheesewaf.yaml after this step; never write setup state back to configs/.
...
The default admin listener is `127.0.0.1:9443`. On the server (or over an SSH tunnel) open `http://127.0.0.1:9443/setup`. Every setup mutation, including loopback requests, requires `X-CheeseWAF-Setup-Token`; read the complete one-time URL from the protected runtime `setup.url` file (or set `CHEESEWAF_SETUP_TOKEN` before startup). The startup log contains only the base URL, file path, and a non-secret receipt. From another host, the remote `SERVER_IP:9443` stays closed until setup explicitly chooses a public admin strategy.
...
# View logs and retrieve the protected initial setup URL
docker compose logs -f cheesewaf
```

#### Step 3: Access Admin Interface

- Open `https://127.0.0.1:9443/setup` on the Docker host (admin uses HTTPS with a self-signed certificate in Docker).
- From another machine, first create an SSH tunnel such as `ssh -L 9443:127.0.0.1:9443 <user>@<server>`, then open the same local URL. Change the bind address only with an explicit access-control plan.
- Read the one-time onboarding URL from the protected runtime `setup.url` file; container logs show only its path and a non-secret receipt.
- Data and logs persist in `cheesewaf-data` and `cheesewaf-logs` volumes across container restarts.

Administrator usernames use the exact value entered. The server rejects leading, trailing, or embedded whitespace, control characters, invisible characters, and non-ASCII characters; it never trims or lowercases a username. If an older database contains a non-canonical username, normal user updates and setup cannot silently repair it. Use `cheesewaf user repair-username USER_ID NEW_USERNAME --reason "repair historical username"` after checking the immutable user ID, then sign in with the new exact username.
...
1. Download `cheesewaf-amd64-windows-*.exe` or `cheesewaf-arm64-windows-*.exe`.
2. Run it as `cheesewaf.exe`:

```powershell
$exe = (Get-ChildItem .\cheesewaf-amd64-windows-*.exe | Where-Object { $_.Name -notlike '*-setup.exe' } | Select-Object -First 1).FullName  # use arm64 on ARM64 Windows
& $exe setup
...
The release pipeline embeds the Web UI into the single-file executable, but that artifact does not include external `configs/`, `web/`, or the GUI controller. Use the portable ZIP or NSIS package when you need those companion files; source builds must run `bash scripts/ci/build-web.sh` before `go build` or `/setup` will return 404.

#### Option B: Portable folder (Zip)

1. Download `cheesewaf-amd64-windows-*.zip` or `cheesewaf-arm64-windows-*.zip` and extract it (for example `D:\CheeseWAF`).
2. Run:
...
1. Run `cheesewaf-amd64-windows-*-setup.exe` or `cheesewaf-arm64-windows-*-setup.exe`.
...
- Prefer copying and opening the complete one-time URL from the protected `setup.url` file, for example `http://127.0.0.1:9443/setup#setup_token=...` (use `https://127.0.0.1:9443/setup#setup_token=...` in Docker). The wizard removes the fragment from the address bar and sends the token only as `X-CheeseWAF-Setup-Token`.
- If you open bare `/setup`, the page provides a manual Token fallback. Paste the value from the protected `setup.url` file; it is kept only in page memory and cleared from the input after submission. Never put the token in a query parameter or include it in screenshots, tickets, or logs.
- Create your administrator account. Save the administrator password and protect the runtime configuration/certificate backup; the current setup wizard does not display or export a portable system master key.
...
In **AI Settings**:
1. **Endpoint**: Configure `ai.api_base` with your LLM provider endpoint (e.g., `https://api.openai.com/v1`). The legacy key `ai.base_url` is not read by the current release.
2. **API Key & Model**: Enter credentials and select the target model.
3. **Auto-Agree**: This is the site-level `sites[].waf.semantic_policy.auto_agree` switch. The legacy key `ai.auto_agree` has no effect.
...
The default trusted CIDR list is empty. If the deployment is behind a trusted proxy, add only explicitly controlled networks such as `127.0.0.1/32` or `::1/128`. `CHEESEWAF_ADAPTER_TOKEN`, when configured, authenticates adapters through the dedicated `X-CheeseWAF-Adapter-Token` header. It is not a Bearer token and must not be sent as `Authorization: Bearer ...`.

When a request body exceeds the configured `max_body_bytes`, the request is rejected with HTTP `413 Request Entity Too Large`. The supported WAF modes are `block`, `monitor`, and `off`; `log` is accepted only as a compatibility alias and is normalized to `monitor`.
...
| **Detection Core** | In-process AST semantic analyzer, dynamic fingerprinting, sharded sliding-window rate limiter |
| **Review Engine** | Asynchronous task queues, standard Chat Completions / Messages protocol adapters |
| **Storage** | `storage.profile: temporary` with embedded SQLite management store; optional `storage.postgresql` access-log sink; Redis Bot challenge backend rejected; builtin single-node cluster path, with configured etcd/native-raft coordinators not wired |
...
The frontend dependency `@agent-eyes/agent-eyes`, code-inspector, `codex-acp`, and related layout-inspection scripts are local development/test tooling only. They must never enter production JavaScript, release packages, Docker runtime images, or runtime dependencies. Release builds must use `bash scripts/ci/build-web.sh`; it skips dependency lifecycle scripts, sets `CHEESEWAF_AGENT_EYES=0`, and fails if Agent-tool markers appear in the generated assets.

---

## Development & Testing

### Offline CRP verification

Verify a local `.crp` ZIP without installing, activating, or contacting a network. Trust roots and source registrations must be supplied explicitly as JSON files, and `--now` is required for deterministic signature checks:

```bash
cheesewaf crp verify --package ./plugin.crp --trust-roots ./trust-roots.json --sources ./sources.json --now 2026-09-06T12:00:00Z
```

The command prints only the manifest identity, signature status, and artifact size.

### Offline CRP staging

After verification, a trusted package can be persisted into an explicit local staged slot without activating or executing it:
...
This command re-runs offline admission, stores content-addressed artifact and metadata files with owner-only permissions, and reports the staged revision. It never promotes the package. Signer classes that require administrator confirmation are rejected because the CLI does not yet implement the password, 10-second warning, third confirmation, and durable audit flow. Treat this as a local staged-state foundation, not as a plugin installer or OTA client.

### Explicit temporary HTTPS probe

The only mounted temporary-egress entry point is an administrator-run, one-shot
probe. It is not a background worker, downloader, or `serve` feature; when it returns, it explicitly revokes the one-shot lease and closes the temporary session:

```bash
cheesewaf temporary-online probe --help
```

The command requires `--administrator`, `--password-stdin`, plugin ID/version,
a lowercase HTTPS hostname, an exact `sha256:` TLS leaf pin, and bounded TTL
and byte limits. It permits only `HEAD` or `GET` against a relative path,
resolves once, dials the selected public IP directly, and writes metadata-only
audit records to `<data-dir>/audit/netlease.jsonl`. Supply the administrator
password through standard input from a trusted local terminal or secret source;
there is deliberately no password flag. CWEDP, CRP activation, the control
plane, and `cheesewaf serve` are not yet wired to this broker.

### Get Started acceptance harness

Run the bounded local smoke that copies `configs/cheesewaf.yaml` into a temporary runtime directory, builds the Go binary, starts the `temporary` profile on loopback, checks `/health/ready` and setup initialization, and verifies that `production` fails closed without a SQLite fallback:

```bash
bash scripts/acceptance/get-started.sh
```

For a static/contract-only check (no build and no service):

```bash
bash scripts/acceptance/get-started.sh --static-contract
```

The harness never starts PostgreSQL, Redis, or other persistent external services, and removes its temporary directory on exit.
...
# 2. Build Web frontend static assets (release builds exclude Agent tooling)
bash scripts/ci/build-web.sh

# 3. Build backend binary
go build -o bin/cheesewaf ./cmd/cheesewaf

# 4. Create a runtime config copy and run (do not write into configs/cheesewaf.yaml)
mkdir -p ./data/config
