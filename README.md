...
- Frontend build output uses stable Vite/Rolldown vendor chunks for React, Appica, Three.js, visualization, runtime, and UI utility dependencies. Production source maps are disabled unless `VITE_SOURCEMAP=true` is set, keeping the released Web console payload small, while the large Three.js dependency stays isolated to the attack-map path.
