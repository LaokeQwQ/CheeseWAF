...
- Block/error pages are now runtime-configurable instead of preview-only. The proxy renders a polished built-in HTML template by default, every block response exposes the same `X-CheeseWAF-Trace-ID` shown in the page body, and the Web console can enable built-in templates, upload/admin-edit custom Go `html/template` HTML, validate it server-side, persist it to YAML, and hot-reload the proxy renderer without a restart. Management API errors and frontend runtime errors also show a Trace ID for cross-checking browser reports against backend logs.
