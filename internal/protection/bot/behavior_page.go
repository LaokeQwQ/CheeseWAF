...
func behaviorPrompt(locale string, kind captcha.BehaviorType, prompt string) string {
	...
	static := map[captcha.BehaviorType][2]string{
		...
		captcha.BehaviorCurveSlider:   {"拖动实线曲线，使其与虚线引导完全重合", "実線の曲線をドラッグして破線ガイドに重ねてください"},
...
function setupContinuous(){if(!needImage())return;if(type==="curve_draw"){bindTrack(()=>{})}else if(type==="curve_slider"){if(!piece.getAttribute("src")){showUnavailable();return}overlay.classList.add("hidden");piece.classList.remove("hidden");piece.classList.add("curve-piece");slider.classList.remove("hidden");slider.max="10000";slider.value="5000";slider.setAttribute("aria-label",P.prompt||S.text.Hint);let sliderStarted=false;const minDuration=Math.max(0,Number((P.track||{}).min_duration_ms)||0);const begin=()=>{if(terminal||sliderStarted)return;sliderStarted=true;start=performance.now()},update=()=>{const value=clamp(Number(slider.value));response.point={x:value,y:5000};piece.style.transform="translateX("+(((value-5000)/5000)*16)+"%)";if(!sliderStarted){return}const duration=Math.max(minDuration,Math.round(performance.now()-start)),middle=Math.round((5000+value)/2);response.track=[{x:5000,y:5000,t:0,type:"down"},{x:middle,y:5000,t:Math.round(duration/2),type:"move"},{x:value,y:5000,t:duration,type:"up"}];response.duration_ms=duration},finish=()=>{if(!sliderStarted)return;update();sliderStarted=false},keys=new Set(["ArrowLeft","ArrowRight","ArrowUp","ArrowDown","Home","End","PageUp","PageDown"]);slider.onpointerdown=begin;slider.onpointerup=finish;slider.onpointercancel=()=>{if(!sliderStarted)return;sliderStarted=false;delete response.track;delete response.duration_ms};slider.onkeydown=e=>{if(keys.has(e.key)&&!e.repeat)begin()};slider.oninput=update;slider.onchange=update;slider.onblur=finish;update()}else if(type==="shape_slider"){if(!piece.getAttribute("src")){showUnavailable();return}overlay.classList.add("hidden");piece.classList.remove("hidden");slider.classList.remove("hidden");slider.max="10000";slider.value="0";slider.setAttribute("aria-label",P.prompt||S.text.Hint);const width=Math.max(1,Number(P.width)||320),height=Math.max(1,Number(P.height)||180),pieceSize=Math.max(1,Number(P.piece_size)||64),startY=Number(P.piece_y!=null?P.piece_y:Math.round((height-pieceSize)/2)),trackAngle=Math.max(-44,Math.min(44,Number(P.track_angle)||0)),travel=Math.max(1,width-pieceSize),minDuration=Math.max(0,Number((P.track||{}).min_duration_ms)||0);let sliderStarted=false;const pointFor=v=>{const xPx=(v/10000)*travel,yPx=startY+xPx*Math.tan(trackAngle*Math.PI/180);return{x:v,y:clamp(((yPx+pieceSize/2)/height)*10000)}};const place=v=>{const xPx=(v/10000)*travel,yPx=startY+xPx*Math.tan(trackAngle*Math.PI/180);piece.style.left=(xPx/width*100)+"%";piece.style.top=(yPx/height*100)+"%";piece.style.width=(pieceSize/width*100)+"%";piece.style.height=(pieceSize/height*100)+"%";piece.style.transform="none"};const begin=()=>{if(terminal||sliderStarted)return;sliderStarted=true;start=performance.now()},update=()=>{const value=clamp(Number(slider.value)),pt=pointFor(value);response.point=pt;place(value);if(!sliderStarted)return;const duration=Math.max(minDuration,Math.round(performance.now()-start)),mid=Math.round(value/2);response.track=[{x:0,y:pointFor(0).y,t:0,type:"down"},{x:mid,y:pointFor(mid).y,t:Math.round(duration/2),type:"move"},{x:value,y:pt.y,t:duration,type:"up"}];response.duration_ms=duration},finish=()=>{if(!sliderStarted)return;update();sliderStarted=false},keys=new Set(["ArrowLeft","ArrowRight","ArrowUp","ArrowDown","Home","End","PageUp","PageDown"]);slider.onpointerdown=begin;slider.onpointerup=finish;slider.onpointercancel=()=>{if(!sliderStarted)return;sliderStarted=false;delete response.track;delete response.duration_ms};slider.onkeydown=e=>{if(keys.has(e.key)&&!e.repeat)begin()};slider.oninput=update;slider.onchange=update;slider.onblur=finish;update()}else if(type==="restore_slider"){restoreStrip.classList.remove("hidden");slider.classList.remove("hidden");slider.max="10000";slider.value="5000";const initialOffset=Number(P.initial_offset||0),maxOffset=Math.max(0,Number(P.max_offset)||0),minDuration=Math.max(0,Number((P.track||{}).min_duration_ms)||0);const update=()=>{const value=clamp(Number(slider.value)),duration=Math.max(minDuration,Math.round(performance.now()-start));response.offset=Math.round((initialOffset+(value/10000)*2*maxOffset-maxOffset)*100)/100;response.track=[{x:5000,y:5000,t:0,type:"down"},{x:value,y:5000,t:duration,type:"up"}];response.duration_ms=duration;strip.style.transform="translateX("+response.offset+"%)"};slider.onpointerdown=()=>{if(terminal)return;start=performance.now()};slider.oninput=update;slider.onchange=update;update()}}
