...
var messages = map[string]map[string]string{
	"en": {
		...
		"setup.probe.incomplete":  "Probe timed out: falling back to the low profile",
		...
		"setup.profile.desc.medium": "medium - >= 3 cores and >= 4 GB RAM; balanced defaults",
	...
	"zh-CN": {
		...
		"setup.probe.incomplete":  "探测超时：已回退到 low 档位",
		...
		"setup.profile.desc.low":    "low - ≤ 2 核或 ≤ 2 GB 内存；开启采样、降低请求体上限",
		"setup.profile.desc.medium": "medium - ≥ 3 核且 ≥ 4 GB 内存；均衡默认值",
		"setup.profile.desc.high":   "high - ≥ 4 核、≥ 8 GB 内存且磁盘较快；最严格默认值",
