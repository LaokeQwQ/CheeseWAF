...
func TestAnalyzerReadinessMatrix(t *testing.T) {
	cases := []struct {
		...
		{name: "sqli-having-count-inference", method: http.MethodGet, target: "/search?q=1%20having%201%3D1--", category: "sqli"},
		{name: "sqli-rlike-binary-probe", method: http.MethodGet, target: "/search?q=1%20and%20database()%20rlike%20binary%200x5e61646d696e", category: "sqli"},
		{name: "sqli-procedure-analyse-enumeration", method: http.MethodGet, target: "/search?q=1%20procedure%20analyse(extractvalue(rand(),concat(0x3a,version())),1)", category: "sqli"},
		{name: "sqli-oracle-dbms-lock-sleep", method: http.MethodPost, target: "/api/report", contentType: "application/x-www-form-urlencoded", body: "id=1%27%7C%7Cdbms_lock.sleep(5)%7C%7C%27", category: "sqli"},
		{name: "sqli-tsql-openrowset-side-effect", method: http.MethodPost, target: "/api/report", contentType: "application/x-www-form-urlencoded", body: "id=1%3Bselect%20*%20from%20openrowset(%27SQLOLEDB%27,%27server=x%3Buid=sa%3Bpwd=p%27,%27select%201%27)--", category: "sqli"},
		...
		{name: "xss-srcdoc-entity-script", method: http.MethodGet, target: "/?frame=%3Ciframe%20srcdoc%3D%22%26lt%3Bscript%26gt%3Balert(1)%26lt%3B%2Fscript%26gt%3B%22%3E", category: "xss"},
		...
		{name: "ssrf-hex-integer-loopback", method: http.MethodGet, target: "/fetch?url=http://0x7f000001/admin", category: "ssrf"},
		{name: "ssrf-dynamic-dns-loopback", method: http.MethodGet, target: "/fetch?url=http://127.0.0.1.nip.io/admin", category: "ssrf"},
		...
		{name: "ssrf-gopher-redis-loopback", method: http.MethodGet, target: "/fetch?url=gopher://127.0.0.1:6379/_INFO", category: "ssrf"},
		{name: "ssrf-dict-metadata-service", method: http.MethodGet, target: "/fetch?url=dict://169.254.169.254:11211/stat", category: "ssrf"},
		{name: "ssrf-scheme-relative-loopback", method: http.MethodGet, target: "/fetch?target=//127.0.0.1:8080/admin", category: "ssrf"},
		{name: "ssrf-bare-metadata-host-json", method: http.MethodPost, target: "/fetch", contentType: "application/json", body: `{"endpoint":"169.254.169.254/latest/meta-data"}`, category: "ssrf"},
		{name: "ssrf-userinfo-bare-loopback", method: http.MethodGet, target: "/fetch?webhook=token@127.0.0.1:8080/admin", category: "ssrf"},
		...
		{name: "nosqli-json-schema-query", method: http.MethodPost, target: "/api/search", contentType: "application/json", body: `{"filter":{"$jsonSchema":{"required":["role"],"properties":{"role":{"bsonType":"string"}}}}}`, category: "nosqli"},
...
func TestAnalyzerReadinessBenignMatrix(t *testing.T) {
	cases := []struct {
		...
		{name: "sql-inference-documentation", target: "/docs", contentType: "application/json", body: `{"text":"Database tutorials can mention HAVING, RLIKE, BINARY, and PROCEDURE ANALYSE as SQL syntax without user-controlled predicates."}`},
		{name: "sql-dialect-side-effect-documentation", target: "/docs", contentType: "application/json", body: `{"text":"Oracle DBMS_LOCK.SLEEP and SQL Server OPENROWSET are documented here as database hardening topics, not executed user input."}`},
		{name: "dynamic-dns-documentation", target: "/docs", contentType: "application/json", body: `{"text":"Security docs discuss nip.io and sslip.io loopback examples without asking this service to fetch them."}`},
		{name: "bare-metadata-host-documentation", target: "/docs", contentType: "application/json", body: `{"text":"Security docs may mention 169.254.169.254/latest/meta-data as an example without asking this service to fetch it."}`},
		...
		{name: "gopher-dict-documentation", target: "/docs", contentType: "application/json", body: `{"text":"Security documentation may mention gopher://127.0.0.1:6379 and dict://169.254.169.254 examples without asking this service to fetch them."}`},
		...
		{name: "mongodb-json-schema-documentation", target: "/docs", contentType: "application/json", body: `{"text":"MongoDB docs can describe $jsonSchema validation for defenders without sending it as a live query filter."}`},
