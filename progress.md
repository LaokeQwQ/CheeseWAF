...
M1 已完成基础落地：配置层新增默认单机的 `deployment` / `cluster` 模型，校验会阻止“两台 WAF 节点被误标为完整高可用”；后端新增声明式对象模型和单机对象存储；CLI 新增 `cheesewaf cluster status/init/export`；API 新增集群状态与健康端点；Web 控制台新增“集群”菜单和状态页。当前仍未实现 M2-M4 的临时 SSH/Ansible 部署、一次性加入令牌、节点证书签发、真实多链路心跳、监控节点运行时、Raft/etcd 多数确认、配置对象调和和生产流量调度。
